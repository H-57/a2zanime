...
for(let i=0;i<Aname.length;i++){
  ...
  const data = await (
    await fetch(`https://animeweb-d093d-default-rtdb.firebaseio.com/${Aname[i]}/season.json`)
  ).json();
...
export const getStaticProps = async (context) => {
    ...
    console.log(AnimeName)
      let res = await fetch(
        `https://animeweb-d093d-default-rtdb.firebaseio.com/${AnimeName}/${Season}.json`
      );
          ...
          return (
            <>
              ...
              <ul id="video" className="video">
                {data.map((elem, index) => {
                  return <VideoCards  key={index} title={elem.title} image={elem.image} number={index} />;
                })}
              </ul>
             <style jsx>{`
